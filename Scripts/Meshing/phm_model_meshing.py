...
def phm_model_meshing(base_path: str, suffix_name: str, electrode_attributes: dict, max_radius=5):
    ...
    ##### Import th model files to create the mesh
    skin_stl.add_attribute("Skin")
    skull_stl.add_attribute("Skull")
    skin_stl.set_attribute("Skin", np.zeros(skin_stl.num_vertices) + 1)
    skull_stl.set_attribute("Skull", np.zeros(skull_stl.num_vertices) + 2)
