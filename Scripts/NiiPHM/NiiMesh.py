...
class NiiMesh:
    ...
    def assign_intensities(self, assign_values_per_region: bool, assign_index: bool=False, values_to_assign=None, unwanted_region_ids: list=None):
        ...
        if assign_values_per_region == False:
            assert len(self.intensities) == len(values_to_assign), 'The provided intensity values shall be the same length as the points.'

        for index, id in enumerate(np.unique(self._mesh_pts[self.__index_array_name])):
            pts_loc = np.where(self._mesh_pts[self.__index_array_name] == id)[0]
            if assign_index:
                self.intensities[pts_loc] = 0 if (id in unwanted_region_ids) else id
            else:
                if assign_values_per_region:
                    self.intensities[pts_loc] = 0 if (id in unwanted_region_ids) else values_to_assign[index]
                else:
                    self.intensities[pts_loc] = 0 if (id in unwanted_region_ids) else values_to_assign[pts_loc]
